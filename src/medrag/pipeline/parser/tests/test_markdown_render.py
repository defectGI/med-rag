...
def test_image_with_ocr():
    ...
    assert _md(img).strip() == (
        "![](img_a1)\n\n*OCR (unverified image text, may be unreliable): "
        "Bus coupler*")


def test_image_with_ocr_shows_confidence_when_known():
    img = ImageBlock(id="", image_index=1, image_id="img_a1",
                     ocr_text="Bus coupler", ocr_meaningful=True,
                     ocr_confidence=0.82)
    assert _md(img).strip() == (
        "![](img_a1)\n\n*OCR (unverified image text, may be unreliable — "
        "confidence 0.82): Bus coupler*")
...
def test_table_description_appended():
    t = _simple_table()
    t.description = "A 2x2 grid"
    md = _md(t)
    # anchored label: between two consecutive tables a bare italic line is
    # ambiguous about which table it describes
    assert md.strip().endswith("*Table above: A 2x2 grid*")
...
def test_edge_whitespace_rendered_outside_marks():
    # Hygiene: a run whose text carries a leading space must not
    # put that space inside the ** delimiters (flanking-invalid Markdown that
    # renders literal asterisks).
    p = ParagraphBlock(id="", text="support@ example.com", runs=[
        InlineRun("support@", ()),
        InlineRun(" example.com", (Mark.BOLD,))])
    assert _md(p).strip() == "support@ **example.com**"


def test_all_whitespace_marked_run_left_bare():
    p = ParagraphBlock(id="", text="a b", runs=[
        InlineRun("a", ()), InlineRun(" ", (Mark.BOLD,)), InlineRun("b", ())])
    assert _md(p).strip() == "a b"


def test_edge_whitespace_outside_link():
    p = ParagraphBlock(id="", text="see here now", runs=[
        InlineRun("see", ()),
        InlineRun(" here ", (), "https://x.example"),
        InlineRun("now", ())])
    assert _md(p).strip() == "see [here](https://x.example) now"
