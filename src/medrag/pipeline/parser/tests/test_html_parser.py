...
def test_heading_id_attribute_becomes_anchor_id(tmp_path):
    doc = _parse(tmp_path, '<h2 id="installation">Installation</h2>')
    h = next(b for b in doc.blocks if isinstance(b, HeadingBlock))
    assert h.anchor_id == "installation"


def test_heading_without_id_has_no_anchor_id(tmp_path):
    doc = _parse(tmp_path, "<h2>Installation</h2>")
    h = next(b for b in doc.blocks if isinstance(b, HeadingBlock))
    assert h.anchor_id is None
    assert "anchor_id" not in h.to_dict()


def test_heading_id_with_inline_marks(tmp_path):
    doc = _parse(tmp_path, '<h1 id="hi">Hello <b>World</b></h1>')
    h = next(b for b in doc.blocks if isinstance(b, HeadingBlock))
    assert h.anchor_id == "hi"
    assert h.text == "Hello World"
