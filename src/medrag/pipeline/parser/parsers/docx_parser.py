...
Byte offsets (best-effort): each body-level paragraph/table is matched
...
Follow-up (not yet done, tracked here rather than silently skipped): docx has no
hyperlink support at all today -- `InlineRun.link` is never set and `HeadingBlock.
anchor_id` is never populated, unlike html/markdown. Filling both in would need (1)
reading `w:bookmarkStart` above/inside a heading into `anchor_id`, and (2) reading
`w:hyperlink[@w:anchor]` (internal, "#name") and `w:hyperlink[@r:id]` (external, via
`document.part.related_parts`) into `InlineRun(text=..., link=...)` in `_walk_para`.
This is an independent, larger chunk of work than the html/markdown side and is left
...
# a:graphicData wraps EVERY drawing object (pictures, charts, SmartArt) --
# real pictures reach it via the picture namespace uri and fall through to
# the generic recursion below (finding their own a:blip deeper in the tree,
# unaffected). Chart/diagram uris are the two this parser now recognizes by
# name instead of silently falling through with nothing found at all (the
# gap this task closes -- see chart_extract.py).
_A_GRAPHIC_DATA = qn("a:graphicData")
_CHART_NS = "http://schemas.openxmlformats.org/drawingml/2006/chart"
_DIAGRAM_NS = "http://schemas.openxmlformats.org/drawingml/2006/diagram"
_C_CHART = "{" + _CHART_NS + "}chart"
_DGM_REL_IDS = "{" + _DIAGRAM_NS + "}relIds"
_R_ID = qn("r:id")
...
@dataclass
class _ImageEmit:
    """One `<imageN>` marker's worth of data collected while walking a
    paragraph -- a real picture (`part`/`mime` set) or a native chart/
    SmartArt (`visual_type`/`visual_type_source` set instead, no `part`:
    there is no media-part locator for either, only structural/heuristic
    metadata -- see `_walk_para`'s docstring)."""
...
def _walk_para(p, document, img_n) -> tuple[list[InlineRun], list[_ImageEmit]]:
    ...
    (w:t) and Office Math text (m:t; unmarked), both modern (a:blip r:embed)
    and legacy/OLE (v:imagedata r:id) images, and native charts/SmartArt (a:
    graphicData whose uri names the DrawingML chart/diagram schema -- see
    `_A_GRAPHIC_DATA` above). A chart is read deterministically from its own
    c:chart part (chart_extract.py, no model call); SmartArt has no such
    schema to extract from, so it's tagged "block_diagram" from the XML
    element alone (certain, not a guess) with no further content -- both used
    to fall through every check here and vanish with zero trace. Under
    mc:AlternateContent only the Choice branch is walked, so an object
    shipping both renderings isn't counted twice. Adjacent same-mark runs are
    merged and the sequence is edge-stripped.
    """
    runs: list[InlineRun] = []
    images: list[_ImageEmit] = []
    ...
    def emit_image(rid: str | None) -> None:
        ...
        try:
            ...
            pass
        images.append(_ImageEmit(image_index=img_n[0], part=part_path, mime=mime))

    def emit_chart(rid: str | None) -> None:
        img_n[0] += 1
        runs.append(InlineRun(f"<image{img_n[0]}>"))
        desc = None
        if rid and get_config().visual.extract_charts:
            try:
                part = document.part.related_parts[rid]
                data = extract_chart(part.blob)
                if data is not None:
                    desc = describe_chart(data)
            except KeyError:
                pass  # relationship unresolvable -- still tag the type
        images.append(_ImageEmit(image_index=img_n[0], visual_type="chart",
                                 visual_type_source="structural",
                                 description=desc,
                                 description_source="structural" if desc else None))

    def emit_smartart(rid: str | None) -> None:
        img_n[0] += 1
        runs.append(InlineRun(f"<image{img_n[0]}>"))
        desc = None
        if rid and get_config().visual.extract_smartart:
            try:
                part = document.part.related_parts[rid]
                texts = extract_smartart(part.blob)
                if texts is not None:
                    desc = describe_smartart(texts)
            except KeyError:
                pass  # relationship unresolvable -- still tag the type
        images.append(_ImageEmit(image_index=img_n[0], visual_type="block_diagram",
                                 visual_type_source="heuristic",
                                 description=desc,
                                 description_source="structural" if desc else None))
    ...
    def visit(el, marks: tuple[Mark, ...]) -> None:
        ...
        if tag == _V_IMAGEDATA:
            ...
            return
        if tag == _A_GRAPHIC_DATA:
            uri = el.get("uri")
            if uri == _CHART_NS:
                chart_el = el.find(_C_CHART)
                emit_chart(chart_el.get(_R_ID) if chart_el is not None else None)
                return
            if uri == _DIAGRAM_NS:
                rel_ids = el.find(_DGM_REL_IDS)
                emit_smartart(rel_ids.get(_R_DM) if rel_ids is not None else None)
                return
            # Any other graphicData (pictures included) -- fall through to
            # the generic recursion below so a nested a:blip is still found.
        if tag == _W_TBL:
        ...
        if child.tag == qn("w:p"):
            ...
            for ref in images:
                blocks.append(ImageBlock(
                    id="", image_index=ref.image_index,
                    locator={"part": ref.part} if ref.part else {}, mime=ref.mime,
                    visual_type=ref.visual_type,
                    visual_type_source=ref.visual_type_source,
...
def _tr_is_header(tr) -> bool:
    """True when the row carries w:trPr/w:tblHeader (Word's "repeat as header
    row" property; an absent w:val means on, "false"/"0" means off)."""
    ...
    if th is None:
        return False
    return (th.get(qn("w:val")) or "true").lower() not in ("false", "0")
...
def _build_docx_table(tbl, document, img_n, depth: int = 0) -> TableData:
    ...
    # Leading rows flagged "repeat as header" (w:tblHeader) become header_rows.
    # The property being absent doesn't prove the table is headerless (authors
    # rarely set it), so zero marked rows stays None (unknown), not 0.
    n_header = 0
    for tr in trs:
        if not _tr_is_header(tr):
            break
        n_header += 1
    return TableData(n_rows=nrows, n_cols=ncols, cells=cells, merges=merges,
                     header_rows=n_header or None)
...
_TITLE_STYLE = re.compile(r"title|subtitle|heading|head|ba[şs]l[ıi]k", re.IGNORECASE)
...
class DocxParser(BaseParser):
    ...
    def parse(self, raw_path: str | Path, doc_id: str) -> ParsedDocument:
        ...
        for i, (kind, obj) in enumerate(items):
            ...
            for ref in images:
                blocks.append(ImageBlock(
                    id=next_id(), image_index=ref.image_index,
                    locator={"part": ref.part} if ref.part else {}, mime=ref.mime,
                    visual_type=ref.visual_type,
                    visual_type_source=ref.visual_type_source,
                    description=ref.description,
                    description_source=ref.description_source, **meta))
