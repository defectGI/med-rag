...
* heading `anchor_id`: an explicit kramdown/attr-list id (`## Section {#custom-id}`)
  wins; otherwise the implicit GitHub-flavored-markdown slug of the heading's own
  text (lowercased, non-word/space/hyphen chars dropped, spaces -> hyphens),
  de-duplicated document-wide the same way GitHub does (`section`, `section-1`,
  `section-2`, ...) — this is what most in-doc `[text](#section)` links actually
  target
...
# Kramdown/attr-list explicit heading id: `## Section {#custom-id}`.
_ATTR_ID = re.compile(r"\s*\{#([A-Za-z0-9_-]+)\}\s*$")
...
def _extract_attr_id(text: str) -> tuple[str, str | None]:
    """Split a trailing kramdown/attr-list `{#custom-id}` off a heading's raw
    (pre-inline-parse) text. Returns (remaining_text, explicit_id_or_None)."""
    m = _ATTR_ID.search(text)
    if not m:
        return text, None
...
def _slugify(text: str) -> str:
    """GitHub-flavored-markdown's implicit heading slug: lowercase, drop
    anything that isn't a word char/space/hyphen, collapse whitespace to a
    single hyphen. This is what GitHub/VSCode's markdown preview generates for
    a heading with no explicit `{#id}`, so a `[text](#some-heading)` link --
    the overwhelming majority of real-world in-doc markdown links, which rely
    on this implicit slug rather than a written-out id -- resolves the same
    way a human reading the rendered preview would expect."""
    s = text.strip().lower()
    s = _SLUG_STRIP.sub("", s)
    s = _SLUG_WS.sub("-", s)
    return s
...
def _dedupe_slug(base: str, counts: dict[str, int]) -> str:
    """GitHub's own de-dup rule: the first heading with a given base slug keeps
    it bare; each later repeat gets a `-N` suffix, counted per BASE slug (not
    per already-emitted final string) -- the exact algorithm github-slugger
    implements, quirks included."""
...
def _heading_anchor_id(clean_text: str, explicit_id: str | None,
                       slug_counts: dict[str, int]) -> str | None:
    """Resolve a heading's `anchor_id`: an explicit `{#id}` wins outright;
    otherwise fall back to the deduped implicit GFM slug of its clean text.
    None if there's no explicit id and the clean text has no slug-able
    characters at all."""
    if explicit_id:
        return explicit_id
    base = _slugify(clean_text)
    return _dedupe_slug(base, slug_counts) if base else None
...
class MarkdownParser(BaseParser):
    ...
    def parse(self, raw_path: str | Path, doc_id: str) -> ParsedDocument:
        ...
        slug_counts: dict[str, int] = {}  # doc-wide implicit-slug dedup (anchor_id)
        ...
        def resolve_cell_images(cell_text: str, row_start: int, row_end: int) -> str:
            ...
            byte range as the image's Span — a best-effort approximation,
        ...
        while i < n:
            ...
            if m:
                raw_text, explicit_id = _extract_attr_id(m.group(2).strip())
                clean, runs = _parse_inline(raw_text)
                blocks.append(HeadingBlock(
                    ...
                    anchor_id=_heading_anchor_id(clean, explicit_id, slug_counts)))
                i += 1
                continue
                ...
                blocks.append(TableBlock(
                    ...
                    table=TableData(n_rows=len(table_rows), n_cols=ncols,
                                    cells=[[_inline_cell(x) for x in r]
                                           for r in table_rows],
                                    # the pipe syntax itself declares row 0 as
                                    # the header (the `---` separator row)
                                    header_rows=1)))
                i = j
                continue
            ...
            if (i + 1 < n and not _LIST.match(text)
                ...
                raw_text, explicit_id = _extract_attr_id(stripped)
                clean, runs = _parse_inline(raw_text)
                blk_end = lines[i + 1][1]
                blocks.append(HeadingBlock(
                    id=next_id(), span=Span(byte_start=start, byte_end=blk_end),
