...
_NAMED_SECTION = re.compile(
    r"^\s*(chapter|section|part|appendix|b[öo]l[üu]m|k[ıi]s[ıi]m|ek)\b", re.IGNORECASE)
...
_MONTH = re.compile(
    ...
    r"may[ıi]s|haziran|temmuz|a[ğg]ustos|eyl[üu]l|ekim|kas[ıi]m|aral[ıi]k)", re.IGNORECASE)
_TIME = re.compile(r"\b\d{1,2}[:.]\d{2}\s*(a\.?m\.?|p\.?m\.?|pm|am)?\b", re.IGNORECASE)
