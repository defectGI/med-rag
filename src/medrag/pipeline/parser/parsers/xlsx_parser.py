...
Byte offsets (best-effort): the TableBlock's Span points at the
...
(c:chart) are read the same deterministic way pptx/docx do (chart_extract.py
-- no model call, straight from the chart's own XML data): a `graphicFrame`
anchor whose `a:graphicData` names the chart schema becomes an ImageBlock
tagged `visual_type="chart"` with a structural `description`. SmartArt-equivalents
have no comparable chart-like schema to extract and remain out of scope (same
call as pptx's SmartArt gap) -- any other `graphicFrame` content is skipped.
"""
...
def _anchor_position(anchor) -> tuple[int | None, int | None]:
    row = col = None
...
def _sheet_images(zf: zipfile.ZipFile, sheet_part: str) -> list[dict]:
    """Visuals anchored on one worksheet: pictures
    ([{"part", "mime", "row", "col"}]) and native charts ([{"row", "col",
    "visual_type": "chart", "visual_type_source": "structural",
    "description"}], `part`/`mime` None -- there is no media-part
    locator for a chart, only its own structural data).

    SmartArt-equivalents and any other `graphicFrame` content stay out of
    scope (see module docstring) -- no comparable chart-like schema to
    extract deterministically from.
    """
    ...
    for anchor in dtree:
        if anchor.tag.rsplit("}", 1)[-1] not in _ANCHOR_TAGS:
            continue
        row, col = _anchor_position(anchor)
        ...
        # `.//` (not a direct-child find): a chart graphicFrame can sit
        # straight under the anchor (openpyxl's own output) or wrapped in
        # mc:AlternateContent (some Excel-authored files) -- both land here.
        graphic_data = anchor.find(f".//{{{_NS_A}}}graphicData")
        if graphic_data is None or graphic_data.get("uri") != _NS_CHART:
            continue  # not a chart -- SmartArt/other graphicFrame content, still skipped
        chart_el = graphic_data.find(f"{{{_NS_CHART}}}chart")
        embed = chart_el.get(f"{{{_NS_R}}}id") if chart_el is not None else None
        desc = None
        if embed and embed in drawing_rels and get_config().visual.extract_charts:
            _typ, chart_part = drawing_rels[embed]
            if chart_part in zf.namelist():
                data = extract_chart(zf.read(chart_part))
                if data is not None:
                    desc = describe_chart(data)
        out.append({"part": None, "mime": None, "row": row, "col": col,
                    "visual_type": "chart", "visual_type_source": "structural",
                    "description": desc,
    ...
    def parse(self, raw_path: str | Path, doc_id: str) -> ParsedDocument:
        ...
        for idx, ws in enumerate(wb.worksheets):
            ...
            if images:
                ...
                for img in images:
                    img_n += 1
                    locator = {"part": img["part"]} if img["part"] else {}
                    ...
                    blocks.append(ImageBlock(
                        ...
                        visual_type_source=img.get("visual_type_source"),
                        description=img.get("description"),
                        description_source=img.get("description_source")))
