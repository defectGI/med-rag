...
Handled:
* h1..h6 -> HeadingBlock; p/div/section/... -> ParagraphBlock. A heading's own
  `id` attribute (if present) becomes `HeadingBlock.anchor_id`, so a same-page
  `<a href="#id">` (see `InlineRun.link` below) can be resolved to it later.
...
class _Builder(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs) -> None:
        ...
        if tag in _HEADINGS:
            ...
            self.heading_id = a.get("id") or None
        elif tag in ("ul", "ol"):
            if not self.list_stack:
                self._list_seq += 1
            ...
            self.tables.append({"rows": [], "row": [], "cell": [],
                                ...
                                "meta": self._list_meta(),
                                # header semantics: one bool per completed row
                                # ("is a header row"), driven by <thead> and
                                # all-<th> rows
                                "row_headers": [], "row_th": [],
                                "in_thead": False})
        elif tag == "thead" and self.tables:
            self.tables[-1]["in_thead"] = True
        elif tag == "tr" and self.tables:
            self.tables[-1]["row"] = []
            self.tables[-1]["row_th"] = []
    ...
    def handle_endtag(self, tag: str) -> None:
        ...
        if tag == "tr" and self.tables:
            ...
            if t["row"]:
                t["rows"].append(t["row"])
                t["row_headers"].append(t["in_thead"] or all(t["row_th"]))
                t["row"] = []
            return
        if tag == "thead" and self.tables:
            self.tables[-1]["in_thead"] = False
            return
    ...
    def _close_table(self) -> None:
        ...
        # Leading rows marked header (in <thead>, or all cells <th>) become
        # header_rows. An all-<td> table stays None: the author simply not
        # using <th> doesn't prove the table is headerless.
        n_header = 0
        for is_header in t["row_headers"]:
            if not is_header:
                break
            n_header += 1
        if n_header:
            table.header_rows = n_header
