...
* images             -> `![alt](image_id)` + an italic, explicitly-unverified OCR
                        line beneath (with the reviewer's confidence when known)
                        when the image carried meaningful OCR text
* tables             -> a GFM pipe table when it is rectangular and every cell is
                        a single paragraph; a raw `<table>` (with rowspan/colspan)
                        when it has merges or a cell holds block content; an LLM
                        description follows as `*Table above: ...*` (explicitly
                        anchored so consecutive tables can't claim it)
...
def _render_run(run: InlineRun, *, html: bool) -> str:
    ...
    Edge whitespace is rendered OUTSIDE the mark delimiters: a run whose text
    starts/ends with a space (e.g. after merging adjacent same-mark words)
    would otherwise produce `**bold **text` -- flanking-delimiter-invalid
    Markdown that renders as literal asterisks (the 131 "support@**
    example.com**" documents)."""
    if run.text != run.text.strip() and (run.marks or run.link):
        lead = run.text[:len(run.text) - len(run.text.lstrip())]
        trail = run.text[len(run.text.rstrip()):]
        core = run.text.strip()
        if not core:  # all-whitespace run: no visible text to mark
            return run.text
        return lead + _render_run(InlineRun(core, run.marks, run.link),
                                  html=html) + trail
    marks = set(run.marks)
...
def _render_gfm(table: TableData) -> str:
    ...
    empty_header = gfm_empty_header_row(ncols)
    if table.header_rows == 0:
        # Known headerless (header_rows=0): GFM requires a header line, but
        # promoting the first DATA row into it would misrepresent the table —
        # emit an empty header and keep every row in the body. None (unknown)
        # keeps the historical first-row-as-header assumption.
        head, body = empty_header, rows
    else:
        head, body = (line(rows[0]) if rows else empty_header), rows[1:]
    out = [head, "| " + " | ".join(["---"] * ncols) + " |"]
    for row in body:
        out.append(line(row))
    return "\n".join(out)
...
def _render_html_table(table: TableData) -> str:
    ...
    Cells in the first `header_rows` rows render as `<th>` (None = unknown,
    no rows promoted)."""
    spans = {(m.row, m.col): (m.rowspan, m.colspan) for m in table.merges}
    out = ["<table>"]
    for r, row in enumerate(table.cells):
        tag = "th" if r < (table.header_rows or 0) else "td"
        out.append("<tr>")
        for c, cell in enumerate(row):
            ...
            if (r, c) in spans:
                ...
                if cs > 1:
                    attrs += f' colspan="{cs}"'
            out.append(f"<{tag}{attrs}>{_cell_to_html(cell)}</{tag}>")
        out.append("</tr>")
    out.append("</table>")
    return "".join(out)
...
def _render_table(block: TableBlock) -> str:
    ...
    if block.description:
        # Label which table the description belongs to: with two tables
        # back-to-back, a bare italic line between them reads as either one's
        # caption (and misleads an LLM consumer). Mirrors the `*OCR: ...*`
        # convention used for images.
        return f"{body}\n\n*Table above: {_escape_md(block.description)}*"
    return body
...
def _ocr_caption(block: ImageBlock) -> str:
    """Label for an image's OCR text. Image OCR is unverified (no text layer to
    check against), so the caption says so plainly -- and appends the reviewer's
    confidence when one is known -- rather than presenting the text as fact."""
    tag = "OCR (unverified image text, may be unreliable"
    if block.ocr_confidence is not None:
        tag += f" — confidence {block.ocr_confidence:.2f}"
    return f"{tag})"
...
def _image_md(block: ImageBlock) -> str:
    ...
    if block.ocr_meaningful and block.ocr_text:
        out += f"\n\n*{_ocr_caption(block)}: {_escape_md(block.ocr_text)}*"
    return out
